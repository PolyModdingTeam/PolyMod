...
public static class Visual
{
	...
	public record SkinInfo(int idx, string id);
	...
	// [HarmonyPostfix]
	// [HarmonyPatch(typeof(Resource), nameof(Resource.UpdateObject))]
	// private static void Resource_UpdateObject(Resource __instance)
	// {
	...
	private static void TerrainRenderer_UpdateGraphics(TerrainRenderer __instance, Tile tile)
	{
		...
		SkinType skinType = tile.data.skinType;
	...
	private static bool StartScreen_OnWeeklyChallengedButtonClick(StartScreen __instance, int id, BaseEventData eventdata)
	{
		...
		List<PopupBase.PopupButtonData> popupButtons = new()
		{
			...
			new(
				...
				callback: (UIButtonBase.ButtonAction)Callback,
				customColorStates: ColorConstants.redButtonColorStates
			)
		};
		void Callback(int id, BaseEventData eventData)
        {
			seenWarningWCPopup = true;
			__instance.OnWeeklyChallengedButtonClick(id, eventdata);
        }
	...
	private static void UpdateVisualPart(UnitVisuals.UnitVisualPart visualPart, string name, string style)
	{
		Sprite? sprite = Registry.GetSprite(name, style) ?? Registry.GetSprite(visualPart.skinPart.name, style);
		if (sprite != null)
		{
		...
		Sprite? outlineSprite = Registry.GetSprite($"{name}_outline", style) ?? Registry.GetSprite($"{visualPart.skinPart.name}_outline", style);
